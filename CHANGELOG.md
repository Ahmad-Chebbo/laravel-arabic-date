...
- `Attributes\ArabicDate` — a `#[ArabicDate(['created_at'])]` class attribute alternative to the `$arabicDate` property, mirroring Laravel's own `#[Fillable]`/`#[Hidden]` attribute-based model configuration. If both are present, the property and the attribute are merged. On Laravel versions without attribute-based model configuration support, the attribute is silently ignored — use the property there instead.
